...
from Myambumy.extensions import mail
...
def send_change_email_email(user,token,to=None):
    send_mail(subject='Change email',to=to,template='emails/change_email',user=user,token=token)

def send_change_password(user,token):
    send_mail(subject='Password Change', to=user.email, template='emails/change_password', user=user, token=token)